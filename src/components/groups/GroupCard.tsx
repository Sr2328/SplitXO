...
import { motion } from "framer-motion";
import { Users, MoreVertical, Pencil, Trash2, UserPlus, Sparkles } from "lucide-react";
import { Button } from "@/components/ui/button";
...
export function GroupCard({
  ...
  return (
    ...
    >
      ...
      <div className="relative h-24 sm:h-28 lg:h-32 bg-gradient-to-br from-emerald-500 via-emerald-600 to-teal-600 p-4 sm:p-5 overflow-hidden">
        {/* Gradient overlay */}
        <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/20" />
        
        {/* Decorative elements */}
        <div className="absolute top-0 right-0 w-24 sm:w-32 h-24 sm:h-32 bg-white/10 rounded-full -mr-12 sm:-mr-16 -mt-12 sm:-mt-16 group-hover/card:scale-110 transition-transform duration-500" />
        <div className="absolute bottom-0 left-0 w-20 sm:w-24 h-20 sm:h-24 bg-white/10 rounded-full -ml-10 sm:-ml-12 -mb-10 sm:-mb-12 group-hover/card:scale-110 transition-transform duration-500" />
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col">
        <h3 className="font-bold text-foreground text-base sm:text-lg mb-1 sm:mb-2 line-clamp-1 group-hover/card:text-primary transition-colors">
          {group.name}
        </h3>
        
        <p className="text-xs sm:text-sm text-muted-foreground line-clamp-2 min-h-[32px] sm:min-h-[40px] leading-relaxed mb-4">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {members.length > 0 ? (
              <div className="flex items-center gap-2">
                <div className="flex -space-x-2">
                  {displayMembers.map((member, idx) => (
                    <Avatar
                      key={member.id}
                      className="h-8 w-8 sm:h-9 sm:w-9 border-2 border-white dark:border-gray-900 hover:scale-110 hover:z-20 transition-transform cursor-pointer shadow-md"
                  ...
                  {remainingCount > 0 && (
                    <div
                      className="h-8 w-8 sm:h-9 sm:w-9 rounded-full border-2 border-white dark:border-gray-900 bg-muted flex items-center justify-center shadow-md hover:scale-110 transition-transform"
                      style={{ zIndex: 0 }}
                      title={`+${remainingCount} more member${remainingCount > 1 ? 's' : ''}`}
                    >
                      <span className="text-xs font-bold text-muted-foreground">+{remainingCount}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-muted flex items-center justify-center shadow-sm">
            ...
            <div className="flex items-center gap-1.5">
              <div className="p-1 sm:p-1.5 rounded-lg bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950 dark:to-teal-950 shadow-sm">
